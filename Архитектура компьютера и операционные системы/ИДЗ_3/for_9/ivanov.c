...
int main(int argc, char *argv[]) {
    ...
    // Инициализация структуры данных только при первом запуске
    if (data->total_parties == 0) {
        memset(data, 0, sizeof(message_t));
        data->total_parties = total_parties;
    }

    // Создание/открытие семафоров для синхронизации
    sem_t *mutex   = sem_open(SEM_MUTEX,   O_CREAT, 0666, 1);
    sem_t *ivanov  = sem_open(SEM_IVANOV,  O_CREAT, 0666, 0);
    sem_t *petrov  = sem_open(SEM_PETROV,  O_CREAT, 0666, 0);

    char msg[MSG_SIZE];
    snprintf(msg, sizeof(msg), "Иванов: начинаю выносить имущество (%d партий)...", total_parties);
    printf("%s\n", msg);
    send_message(msg);
    ...
    for (int party = 1; party <= total_parties && !emergency_stop; ++party) {
        ...
        snprintf(msg, sizeof(msg), "Иванов: вынес партию #%d (%d шт.)", party, data->cur_party_size);
        printf("%s\n", msg);
        send_message(msg);
        sem_post(mutex);
    ...
    snprintf(msg, sizeof(msg), "Иванов: завершил свою часть работы.");
    printf("%s\n", msg);
    send_message(msg);
